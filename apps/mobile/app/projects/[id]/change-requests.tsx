/**
 * Change requests on an active project — its own screen now.
 *
 ...
 *
 * This used to render inline on the project screen, full history and all,
 * whether or not there was anything to see. It's reached from a one-line
 * summary row there now (see app/projects/[id]/index.tsx).
 *
 * The propose-change sheet is rendered as a SIBLING of ScreenScroll, not a
 * child of it — nesting a gorhom BottomSheet inside a ScrollView is what
 * caused sheets on the old combined screen to flash open with nothing
 * pressed. See the note at the top of index.tsx for the full explanation.
 */
...
/** Form state for a new change request — mirrors the API's EDITABLE_FIELDS. */
...
export default function ProjectChangeRequestsScreen() {
  const t = useTheme();
  const { id: projectId } = useLocalSearchParams<{ id: string }>();
  const me = useSession((s) => s.profile?.id);

  const { data: projectData, loading: projectLoading, error: projectError } = useFetch(() =>
    endpoints.getProject<{ project: ProjectFields }>(projectId), { cacheKey: `project:${projectId}` }
  );
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScreenScroll refreshing={refreshing} onRefresh={refresh}>
        {loading || projectLoading ? (
          ...
          <>
            ...
            {pendings.length === 0 && history.length === 0 ? (
              <Card>
                <Txt variant="footnote" tone="muted" center>
                  No change requests yet. Propose one if the terms need to shift.
                </Txt>
              </Card>
            ) : null}
            ...
            {pendings.map((cr) => {
              ...
              return (
                <Card key={cr.id} style={{ gap: t.spacing.sm, borderColor: t.color.brand + '40' }}>
      ...
      <Sheet ref={proposeSheet} title="Propose a change" snapPoints={['75%']}>
