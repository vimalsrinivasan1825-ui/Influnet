...
type StageStatus = 'done' | 'skipped' | 'current';
...
export function ProjectFlow({ project, entries, userId, onPreviewImage }: { project: any; entries: any[]; userId: string | null; onPreviewImage?: (url: string) => void }) {
  ...
  const reached = useMemo(
    () => STAGES.filter((_: Stage, i: number) => i <= (currentIdx < 0 ? 0 : currentIdx)),
    [currentIdx],
  );
  ...
  const initialNodes: Node[] = useMemo(() => reached.map((key: Stage, i: number) => ({
    id: key,
    type: 'stage',
    ...
    deletable: false,
  })), [reached, statusOf, entries]);

  const initialEdges: Edge[] = useMemo(() => reached.slice(1).map((key: Stage, i: number) => ({
    id: `${reached[i]}-${key}`,
    source: reached[i],
    target: key,
    animated: true,
        ...
        {selected ? (
          <>
            ...
            {selectedEntries.length === 0 ? (
              <p className="text-base font-medium text-content-muted">No updates were shared in this stage.</p>
            ) : (
