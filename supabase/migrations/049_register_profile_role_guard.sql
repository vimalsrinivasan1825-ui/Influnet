...
-- Adds a role guard so 'admin' can never be self-registered, even if the route
-- validation is bypassed and the RPC is called directly.
--
-- This is a full CREATE OR REPLACE of register_profile (copied verbatim from
-- 031_onboarding_progress.sql) with ONE addition: the role guard block after
-- the role assignment.
