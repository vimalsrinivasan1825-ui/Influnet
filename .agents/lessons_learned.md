...
- **FCM Unignore & Diagnostic Button**: Removed `google-services.json` from `.gitignore` so EAS Build bundles it into native Android APKs. Added `testPushRegistration()` button in `apps/mobile/app/settings.tsx` to surface exact Expo push token registration errors via native Alert dialogs.
