...
- **YouTube Channel ID Resolution (`lib/youtube`)**: Updated channel resolution regex to scan for explicit channel metadata tags (`itemprop="channelId"`, canonical `rel` links, etc.) before doing a generic channelId string search. This avoids matching recommended, featured, or subscribed channels (like Tamil Gaming) listed on the home tab of the target channel (like A2D Channel).
...
- **YouTube Scraper Incorrect Channel Matching**: Entering a channel handle (like `@a2dchannel`) fetched details and video feed for a different channel (like "Tamil Gaming"). This occurred because `resolveChannel` parsed the first instance of `"channelId":` found in the public page HTML, which matches featured/linked channels on the home page instead of the actual owner's ID.
...
- **Robust YouTube Channel ID Extraction**: Updated `resolveChannel` in `lib/youtube.ts` to query explicit owner metadata tags (`itemprop="channelId"`, canonical `rel` links, `og:url` links) first. This guarantees we match the actual channel owner even if other recommended or featured channels (like Tamil Gaming) are listed on their homepage.
