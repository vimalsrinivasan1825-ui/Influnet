...
- **Android FCM Native Push Setup & On-Device Diagnostics (`apps/mobile/app/settings.tsx`)**: Unignored and committed `google-services.json`, added `check:push` shortcut scripts to web/mobile workspaces, and added an on-screen "Test Push Notifications" diagnostic button to mobile Settings restricted to an allowlist of developer emails and admin accounts to debug FCM token registration directly on physical Android/iOS devices without exposing debug UI to public users.
...
- **FCM Unignore & Restricted Diagnostic Button**: Removed `google-services.json` from `.gitignore` so EAS Build bundles it into native Android APKs. Added `testPushRegistration()` button in `apps/mobile/app/settings.tsx` wrapped in an allowlist check (`DEVELOPER_EMAILS`) to surface exact Expo push token registration errors via native Alert dialogs exclusively for internal developers.
